...
$(document).ready(function(){
    ...
    getLeaderBoard('system', false, true); 
});

// acquire leaderboard table and perform sorting
function getLeaderBoard(name, winner, bypass){
  if (client) {
    try {
      $('.leaderboard-tab').empty(); // empty this\
	$("<tr><th>Rank</th><th>Won</th><th>Distance</th><th>Name</th></tr>").appendTo('.leaderboard-tab');
      var personalRanking = '> 20';
      var relativeRank = -1;
      client.getTable('leaderboard').orderByDescending("is_win").orderByDescending("distance_run").take(20).read().done(function(results){
        var len = results.length;
        for(var i=0;i<len;i++){
          var data = results[i];
          var rank = i+1;
          $("<tr><th>"+rank+"</th><th>"+data.is_win+"</th><th>"+data.distance_run+"</th><th>"+data.name+"</th></tr>").appendTo('.leaderboard-tab');
          var won = (data.is_win == 'true');
          if (won == winner && distance == parseInt(data.distance_run) && data.name == name) {
          	relativeRank == i+1;
          }
        }
        if (relativeRank > -1) {
...
var userName = 'system';
function submitHighScore(name, dist, win) {
  try {
    if (isGameEnded && dist > 0 ) {
      ...
      today = mm+'/'+dd+'/'+yyyy
      var obj={'name':name,'date':today, 'distance_run':dist, 'is_win':win};
      client.getTable("leaderboard").insert(obj).done(function(result){
        getLeaderBoard(userName, win, false);
      });
    }
  } catch(err) {
    console.log(err);
...
$("form").on("submit", function(e) {
  ...
  var name = $('#name').val();
  userName = name;
  submitHighScore(name, distance, isWon);
  document.getElementById('submission').className = "hidden";
});
...
function picking(x,y) {
  var beginGameText = [];
  if (MeshText) {
        beginGameText.push(MeshText);
  }
  var vector = new THREE.Vector3(x,y,1);
  // uses the camera projection matrix to transform it to our 3D world space.
...
function gameOverAnimation () {
  ...
  if (currentTime - timeRecord > 3) {
      ...
      displayText("Press Down to Play Again!", 5, 5, -Math.PI /4);
      if (MeshAwards) {
...
function onKeyDown(event) {
  ...
  else if (keyboard.eventMatches(event,"down")) {    // PERSPECTIVE CHANGE
    if (isGameEnded) {
      // console.log("Restart");
